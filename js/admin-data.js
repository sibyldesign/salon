...
function enterDashboard() {
  ...
  // 細節 2：專員個人登入時，自動隱藏所有店務頁面，僅保留日曆與排班
  if (CURRENT_STAFF_PARAM && CURRENT_STAFF_PARAM !== 'all') {
  ...
  try {
    ...
    backendData = {
      ...
      settings: {
        ...
        packageRules: sCfg.package_rules || [{ name: "暢遊包卡 10 點", service_name: "剪髮造型設計 (含洗吹)", times: 10, price: 4000, deduct_points: 1 }],
    ...
    renderUploadSlots();
    renderInventoryList();
    renderBoardingRoomStatus();
    renderBoardingCareChecklist();
  } catch (err) {
    console.error("fetchDataAndRender 錯誤:", err);
  }
}

// 細節 1：後台儲存設定時，連帶把 theme_color 與 industry_type 寫入 stores 主表，前台即時切換色彩！
async function saveStoreSettings() {
  ...
  try {
    ...
    Swal.fire({ title: '全域設定與前台風格色系已成功同步！', icon: 'success', timer: 1200, showConfirmButton: false });
  } catch (e) {
    Swal.fire('儲存失敗', e.message, 'error');
  }
}

// 細節 4：完工結單自動執行 BOM 耗材扣庫
